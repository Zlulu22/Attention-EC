        ...
        while True:
            ...
            for i in range(self.env_num):
                ...
                if done[i]:
                    ...
                    if ep_idx[i] + ep_len[i] > u_bound:
                        update_len = u_bound - ep_idx[i]
                        self.buffer.rew[ep_idx[i]: u_bound] = self.ep_reward[:update_len]
                        self.ep_reward = self.ep_reward[update_len:]
                        if len(self.ep_reward) > 10000:
                            self.buffer.rew[l_bound: u_bound + 1] = self.ep_reward[:10000]
                            self.ep_reward = self.ep_reward[:10000]
                        self.buffer.rew[l_bound: l_bound + len(self.ep_reward)] = self.ep_reward
                    else:
                        self.buffer.rew[ep_idx[i]: ep_idx[i] + ep_len[i]] = self.ep_reward
