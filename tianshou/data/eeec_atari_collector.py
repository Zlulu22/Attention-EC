...
from tianshou.data.episodic_memory import Memory

class EEEC_Atari_Collector(object):
    ...
    def __init__(
        ...
        ########## result ###########
        self.memory = Memory(args = EEEC_DICT)
    ...
    def collect(
        ...
        while True:
            ...
            for i in range(self.env_num):
                ...
                if done[i]:
                    self.memory.append(self.feature_lists[i], self.reward_lists[i])
                    intrinsic_rewards = self.memory.inquire()
                    ...
                    if ep_idx[i] + ep_len[i] > u_bound:
                        ...
                        self.buffer.rew[ep_idx[i]: u_bound] += intrinsic_rewards[:update_len]
                        intrinsic_rewards = intrinsic_rewards[update_len:]
                        if len(intrinsic_rewards) > 10000:
                            self.buffer.rew[l_bound: u_bound + 1] += intrinsic_rewards[:10000]
                            intrinsic_rewards = intrinsic_rewards[:10000]
                        self.buffer.rew[l_bound: l_bound + len(intrinsic_rewards)] += intrinsic_rewards
                    else:
                        self.buffer.rew[ep_idx[i]: ep_idx[i] + ep_len[i]] += intrinsic_rewards
