...
from tianshou.data import Collector, ReplayBuffer, VectorReplayBuffer, EEEC_MuJoCo_Collector
...
def get_args():
    ...
    parser.add_argument("--score-rate", type=float, default=0.1)              
...
def test_td3(args=get_args()):
    ...
    print(EEEC_DICT)
    train_collector = EEEC_MuJoCo_Collector(policy, train_envs, buffer, exploration_noise=True, EEEC_DICT = EEEC_DICT)
    test_collector = Collector(policy, test_envs)
    train_collector.collect(n_step=args.start_timesteps, random=True)

    # log
    now = datetime.datetime.now().strftime("%y%m%d-%H%M%S")
    args.algo_name = "eeec_td3"
    ...
    with open('results/' + args.task + '/' + args.algo_name.upper() + '/' + now + '_loss.json', 'w') as f:
        json.dump(train_collector.memory.loss_list, f)
